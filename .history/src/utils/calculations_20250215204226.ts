...
export const calculateROI = (inputs: {
  ...
  const totalInvestment = inputs.initialInvestment + 
    (inputs.additionalContribution * 12 * inputs.time); // Assuming monthly contributions
  const netProfit = inputs.finalValue - totalInvestment;
  const roi = (netProfit / totalInvestment) * 100;
  const paybackPeriod = totalInvestment / (inputs.finalValue / inputs.time);
...
export const calculateScientific = (inputs: ScientificInputs): ScientificResults => {
  ...
  try {
    ...
    return {
      result: Number.isFinite(result) ? Number(result.toFixed(8)) : result.toString()
    };
  } catch (error) {
    return {
      result: 0,

...
async function generateSimplifyScreenshot() {
  ...
  try {
    ...
    await page.waitForSelector('.max-w-2xl.mx-auto.mt-12 input[name="simplifyWhole"]', { visible: true })
    await page.waitForSelector('.max-w-2xl.mx-auto.mt-12 input[name="simplifyNum"]', { visible: true })
    await page.waitForSelector('.max-w-2xl.mx-auto.mt-12 input[name="simplifyDen"]', { visible: true })

    // Fill inputs directly
    await page.type('.max-w-2xl.mx-auto.mt-12 input[name="simplifyWhole"]', '2')
    await page.type('.max-w-2xl.mx-auto.mt-12 input[name="simplifyNum"]', '21')
    await page.type('.max-w-2xl.mx-auto.mt-12 input[name="simplifyDen"]', '98')

    // Click calculate button
    const submitButton = await page.waitForSelector('.max-w-2xl.mx-auto.mt-12 button[type="submit"]')
    if (submitButton) {
      await submitButton.click()
    }

    // Wait for result to appear
    await page.waitForSelector('.max-w-2xl.mx-auto.mt-12 .bg-gray-50.p-4.rounded-lg', {
      visible: true,
      timeout: 5000
    })
    ...
    const calculatorElement = await page.$('.max-w-2xl.mx-auto.mt-12 .bg-white.rounded-lg.shadow-lg.p-6')

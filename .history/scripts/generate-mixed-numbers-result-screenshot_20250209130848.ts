  ...
  try {
    ...
    await page.evaluate(() => {
      ...
      if (forms.length >= 2) {
        ...
        const inputs = Array.from(mixedForm.querySelectorAll('input[type="number"]')) as HTMLInputElement[]
        ...
        const button = mixedForm.querySelector('button[type="submit"]') as HTMLButtonElement
        if (button) {
          button.click()
        }
      }
